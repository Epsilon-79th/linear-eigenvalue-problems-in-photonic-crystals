function [lambda_pnt,lambda_re,iter_,X]=PCs_linear_lobpcg_single_gpu(Ds,Bs,Ms,INV,X0,m_conv,shift,tol_lobpcg)
...
lambda=eig(gather(X'*HX),gather(X'*X));
...
while iter<iter_max
    ...
    %{
    U_X=gpuArray(U(1:n_col,:));
    U_W=gpuArray(U(n_col+1:n_col+n_act,:));
    
    if (iter>1)
        U_P=gpuArray(U(n_col+n_act+1:n_col+2*n_act,:));
    end
    %}
